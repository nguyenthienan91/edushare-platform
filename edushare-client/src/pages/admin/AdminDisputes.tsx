...
export default function AdminDisputes() {
  ...
  return (
    <div className='space-y-6 min-h-[calc(100vh-80px)]'>
      <Card>
        <CardContent>
          <Badge className='rounded-full bg-indigo-100 dark:bg-indigo-950/50 text-indigo-700 dark:text-indigo-450 hover:bg-indigo-100 dark:hover:bg-indigo-950/50 border-none'>Trung tâm hòa giải</Badge>
      ...
      {loading && disputes.length === 0 ? (
        ...
        <div className='border border-slate-200 dark:border-slate-850 rounded-2xl p-12 text-center bg-card shadow-sm'>
          <Clock className='w-12 h-12 mx-auto mb-3 text-slate-300 dark:text-slate-650' />
          <h3 className='text-lg font-semibold '>Không có khiếu nại nào</h3>
          <p className='text-sm text-slate-400 dark:text-slate-500 mt-1'>Hệ thống hiện tại sạch bóng các khiếu nại tranh chấp.</p>
        </div>
      ) : (
        <div className='grid gap-6 lg:grid-cols-[380px_1fr] items-start'>
          {/* Left Column: List of Disputes */}
          <Card>
            <CardHeader className='py-5 px-6 border-b border-slate-100 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-900/20'>
          ...
          {currentDispute && (
            <div className='flex flex-col gap-6'>
              <Card>
                {/* Header Detail */}
                <div className='p-6 border-b border-slate-100 dark:border-slate-800 flex flex-col md:flex-row md:items-center md:justify-between gap-4 bg-slate-50/30 dark:bg-slate-900/20'>
                  <div>
                  ...
                  <div className='flex items-center gap-3 bg-white dark:bg-slate-900 p-3 rounded-2xl border border-slate-100 dark:border-slate-800 shadow-sm'>
                    <div className='text-right'>
                      <div className='text-xs text-slate-400 dark:text-slate-500'>Số tiền bị đóng băng</div>
                ...
                <CardContent className='p-6 space-y-6'>
                  ...
                  <div className='grid grid-cols-1 md:grid-cols-2 gap-6'>
                    {/* Left: Member Card */}
                    <Card className='rounded-2xl border-slate-200 dark:border-slate-800 shadow-sm bg-card'>
                      <CardHeader className='pb-3 border-b border-slate-50 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-900/30 p-4'>
                        <CardTitle className='text-sm font-semibold text-rose-700 dark:text-rose-400 flex items-center gap-2'>
                          <span className='flex size-6 items-center justify-center rounded-full bg-rose-100 dark:bg-rose-950/50 text-rose-700 dark:text-rose-400 text-xs font-bold'>1</span>
                          Khiếu nại từ Member
                        </CardTitle>
                      ...
                      <CardContent className='p-4 space-y-4'>
                        <div>
                          <h4 className='text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-wider mb-1'>Lý do & mô tả khiếu nại</h4>
                          <p className='text-sm font-semibold text-slate-800 dark:text-slate-200'>{getReason(currentDispute.reason)}</p>
                        </div>
                        <div>
                          <h4 className='text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-wider mb-2'>Ảnh bằng chứng thành viên gửi</h4>
                          {currentDispute.memberEvidence && currentDispute.memberEvidence.length > 0 ? (
                            <div className='flex flex-col items-center gap-3 w-full'>
                              {currentDispute.memberEvidence.map((src: string, i: number) => (
                                <a
                                  ...
                                  rel='noreferrer'
                                  key={i}
                                  className='relative w-full aspect-video rounded-xl overflow-hidden border border-slate-200 dark:border-slate-800 block bg-slate-50 dark:bg-slate-900 group hover:shadow-md transition-all'
                                >
                                  <img
                                    src={src}
                                    alt='Member Evidence'
                            ...
                            <p className='text-xs text-slate-400 dark:text-slate-500 italic font-medium'>Không đính kèm ảnh.</p>
                          )}
                        </div>
                      </CardContent>
                    </Card>
                    ...
                    <Card className='rounded-2xl border-slate-200 dark:border-slate-800 shadow-sm bg-card'>
                      <CardHeader className='pb-3 border-b border-slate-50 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-900/30 p-4'>
                        <CardTitle className='text-sm font-semibold text-indigo-700 dark:text-indigo-400 flex items-center gap-2'>
                          <span className='flex size-6 items-center justify-center rounded-full bg-indigo-100 dark:bg-indigo-950/50 text-indigo-700 dark:text-indigo-400 text-xs font-bold'>2</span>
                      ...
                      <CardContent className='p-4 space-y-4'>
                        <div>
                          <h4 className='text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-wider mb-1'>Mã giao dịch ký quỹ</h4>
                          <p className='text-sm font-semibold text-slate-800 dark:text-slate-200 truncate'>{currentDispute.transactionId?._id || 'N/A'}</p>
                        </div>
                        <div>
                          <h4 className='text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-wider mb-2'>Ảnh đối chất chủ nhóm gửi</h4>
                          {currentDispute.ownerEvidence && currentDispute.ownerEvidence.length > 0 ? (
                            ...
                            <p className='text-xs text-slate-400 dark:text-slate-500 italic font-medium'>Chủ nhóm chưa cung cấp bằng chứng.</p>
                          )}
                        </div>
                      </CardContent>
                    </Card>
                  ...
                  {currentDispute.status !== 'pending' && (
                    <div className='bg-slate-50 dark:bg-slate-900/40 border border-slate-150 dark:border-slate-800 rounded-2xl p-4 text-sm text-slate-600 dark:text-slate-450'>
                  ...
                  {currentDispute.status === 'pending' && (
                    <div className='pt-4 border-t border-slate-100 dark:border-slate-800 flex flex-wrap items-center justify-end gap-3'>
                      <Button
                        ...
                        className='text-indigo-700 dark:text-indigo-400 border-indigo-200 dark:border-indigo-900 hover:bg-indigo-50 dark:hover:bg-indigo-950/30 rounded-xl'
                        disabled={resolving}
                      >
