...
export default function LandingPage() {
  return (
    <div className="min-h-screen bg-gradient-to-b from-white via-slate-50 to-indigo-50/50 text-slate-900 overflow-x-hidden">
        ...
        <div className="mx-auto grid max-w-7xl items-center gap-14 lg:grid-cols-[1.08fr_0.92fr]">
          <div className="max-w-2xl">
            ...
            <motion.div initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }} className="mt-10 flex flex-col gap-4 sm:flex-row">
              <Link to="/login" className="inline-flex w-full items-center justify-center gap-2 rounded-full bg-gradient-to-r from-indigo-600 to-sky-500 px-7 py-3.5 text-base font-semibold text-white shadow-lg shadow-sky-200 transition-all hover:-translate-y-0.5 hover:shadow-xl sm:w-auto">Khám phá nhóm ngay <ArrowRight className="h-5 w-5" /></Link>
              <Link to="/login" className="inline-flex w-full items-center justify-center gap-2 rounded-full border border-slate-200 bg-white px-7 py-3.5 text-base font-semibold text-slate-700 transition-colors hover:border-slate-300 hover:bg-slate-50 sm:w-auto">Tạo nhóm của bạn</Link>
            </motion.div>
      ...
      <section id="features" className="px-4 py-20 sm:px-6"><div className="mx-auto max-w-6xl"><div className="mx-auto mb-14 max-w-2xl text-center"><p className="mb-3 text-sm font-semibold uppercase tracking-[0.2em] text-sky-600">Điểm nổi bật</p><h2 className="text-3xl font-bold tracking-tight text-slate-900 sm:text-4xl">Được thiết kế để giảm rủi ro và tăng sự tin cậy</h2><p className="mt-4 text-slate-600">EduShare tập trung vào trải nghiệm rõ ràng, hợp tác và thân thiện để sinh viên có thể dùng ngay.</p></div><div className="grid gap-6 md:grid-cols-2 xl:grid-cols-4">{FEATURES.map((feature, index) => { const Icon = feature.icon; return (<motion.div key={feature.title} initial={{ opacity: 0, y: 18 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: index * 0.08 }} className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm transition-all hover:-translate-y-1 hover:shadow-lg"><div className={`mb-5 inline-flex rounded-2xl p-3 ${feature.bg}`}><div className={`rounded-2xl bg-gradient-to-br ${feature.accent} p-2 text-white`}><Icon className="h-5 w-5" /></div></div><h3 className="text-lg font-bold text-slate-900">{feature.title}</h3><p className="mt-2 text-sm leading-relaxed text-slate-600">{feature.desc}</p></motion.div>); })}</div></div></section>
