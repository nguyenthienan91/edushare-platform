      ...
      <div className="space-y-6 bg-[#f0f9ff]">
        ...
        <div className="grid gap-6 lg:grid-cols-[1.1fr_0.9fr]">
          <Card className="rounded-3xl border-slate-200/70 bg-white shadow-sm shadow-sky-100/30">
            <CardHeader>
              <CardTitle className="text-slate-900">Thông tin nhóm</CardTitle>
              <CardDescription>Điền thông tin cơ bản để bắt đầu nhóm chia sẻ của bạn.</CardDescription>
            </CardHeader>
            <CardContent className="space-y-5">
              <div className="grid gap-2">
                <Label className="text-slate-700">Tên nhóm</Label>
                <Input
                  value={groupName}
              ...
              <div className="grid gap-2">
                <Label className="text-slate-700">Loại dịch vụ</Label>
              ...
              <div className="grid gap-2">
                <Label className="text-slate-700">Số lượng thành viên (Slot)</Label>
                <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
                  <div className="mb-3 flex items-center justify-between text-sm text-slate-500">
                    <span>Slot hiện tại</span>
                    <span className="font-medium text-slate-900">{slotCount[0]} người</span>
                  </div>
                  <Slider
                    value={slotCount}
              ...
              <div className="grid gap-2 sm:grid-cols-2">
                <div className="grid gap-2">
                  <Label className="text-slate-700">Tổng giá nhóm</Label>
                  <Input
                    type="number"
                    value={totalPrice}
                ...
                <div className="grid gap-2">
                  <Label className="text-slate-700">Giá mỗi người</Label>
                  <Input
                    value={`${pricePerPerson.toLocaleString('vi-VN')} VND`}
                    readOnly
                    className="h-12 rounded-2xl border-slate-200 bg-white px-4 text-slate-700"
                  />
                </div>
              </div>

              <div className="grid gap-2">
                <Label className="text-slate-700">Lời chào mừng thành viên</Label>
                <textarea
                  value={welcome}
              ...
              <div className="flex flex-wrap gap-3 pt-2">
                <Button variant="outline" className="rounded-full border-indigo-200 bg-white text-indigo-700 hover:bg-indigo-50">
                  Lưu nháp
                </Button>
          ...
          <Card className="rounded-3xl border-slate-200/70 bg-white shadow-sm shadow-sky-100/30">
            <CardHeader>
              <CardTitle className="text-slate-900">Live Preview Card</CardTitle>
              <CardDescription>Xem trước giao diện nhóm của bạn ngay khi chỉnh form.</CardDescription>
            </CardHeader>
            <CardContent>
              <div className="rounded-3xl bg-gradient-to-b from-sky-50 to-white p-5 ring-1 ring-sky-100">
                <div className="flex items-start justify-between gap-4">
                  <div>
                    ...
                    <h3 className="mt-4 text-2xl font-semibold text-slate-900">{groupName || 'Tên nhóm của bạn'}</h3>
                    <p className="mt-1 text-sm text-slate-500">{serviceType === 'video' ? 'Video' : serviceType === 'audio' ? 'Audio' : serviceType === 'ai' ? 'AI' : 'Gaming'} community</p>
                  </div>
                ...
                <div className="mt-5 grid gap-3">
                  <div className="rounded-2xl bg-white p-4 shadow-sm">
                    ...
                    <p className="mt-1 text-lg font-semibold text-slate-900">{pricePerPerson.toLocaleString('vi-VN')} VND / người</p>
                  </div>

                  <div className="rounded-2xl bg-white p-4 shadow-sm">
                    <p className="text-sm font-medium text-slate-700">Lời chào</p>
                    <p className="mt-2 text-sm leading-6 text-slate-500">{welcome || 'Chưa có lời chào mừng.'}</p>
                  </div>

                  <div className="rounded-2xl bg-white p-4 shadow-sm">
                    <p className="text-sm font-medium text-slate-700">Tổng giá</p>
                    <p className="mt-2 text-2xl font-semibold text-slate-900">{totalPrice.toLocaleString('vi-VN')} VND</p>
                  </div>
                </div>
              </div>
            </CardContent>
